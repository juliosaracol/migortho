...
{
    // for all non-free (i.e. occupied) til
        ...
        else if (layout->is_wire_tile(t))
        {
            for (auto& e : layout->get_logic_edges(t))
            {
                ...
                switch (inp_dir_num)
                {
                    ...
                    case 0b1111: { p.inp.emplace(0u, 1u); break; }// NORTH-EAST-SOUTH-WEST
                    default:
                    {
                        std::cout<<"DEFAULT INPUT PARA "<<inp_dir_num<<"\n";
                        throw std::invalid_argument{inp_dir_num+"Unsupported input wire directions in tile"};
                    }
                }
                // numeral representation of direction bitset
                ...
                switch (out_dir_num)
                {
                    ...
                    //JULIO Wires Double
                    case 0b0011: { p.out.emplace(0u, 4u); break; }// SOUTH-WEST
                    case 0b1100: { p.out.emplace(4u, 0u); break; }// NORTH-EAST
                    default:
                    {
                        std::cout<<"DEFAULT OUT PARA "<<out_dir_num<<"\n";
                        throw std::invalid_argument{out_dir_num+"Unsupported output wire directions in tile"};
                    }
                }
