...
void logic_network::substitute() noexcept
{
    ...
    auto add_fan_outs = [this]()
    {
        ...
        //JULIO
    auto transform_maj_to_majx = [this]()
    {
        for (auto&& v : get_vertices())
        {
            if (get_in_degree(v) >= 3u && get_op(v) == operation::MAJ)
            {
                int count =0;
                for (auto&& ie : in_edges(v, true, true))
                {
                    vertex sourceV = get_source(ie);
                    if( get_op(sourceV) != operation::PI){
                        count++;
                    }
                }
                if(count == 3){
                    properties(v) = operation::MAJX;
                }
            }
        }
    };
    ...
    //JULIO
    auto add_fanouts_2_2 = [this]()
    {
        for (auto&& v : get_vertices())
        {
            if (get_op(v) == operation::MAJX )
            {
                std::vector<vertex> ww{};
                std::vector<edge> ve{};
                auto fan_out22 = create_logic_vertex(operation::F2O2);

                for (auto&& ie : in_edges(v, true, true))
                {
                    ww.push_back(get_source(ie));
                    ve.push_back(ie);
                }
                add_edge(ww[0], fan_out22);
                add_edge(ww[1], fan_out22);
                add_edge(fan_out22,v);
